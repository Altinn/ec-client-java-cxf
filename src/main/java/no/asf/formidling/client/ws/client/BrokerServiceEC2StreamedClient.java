 ...
 *
 */
public class BrokerServiceEC2StreamedClient {

    IBrokerServiceExternalEC2Streamed brokerServiceExternalEC2Streamed;
    ...
    protected BrokerServiceEC2StreamedClient(SecurityCredentials credentials) {
        this.credentials = credentials;
        this.objectFactory = new ObjectFactory();
        this.brokerServiceExternalEC2Streamed = getStreamedClient(credentials.getKeyStoreProperties());
    }

    private IBrokerServiceExternalEC2Streamed getStreamedClient(Properties signatureProperties) {
        BrokerServiceExternalEC2StreamedSF service = new BrokerServiceExternalEC2StreamedSF();
        IBrokerServiceExternalEC2Streamed port = service.getCustomBindingIBrokerServiceExternalEC2Streamed(new MTOMFeature());
        BindingProvider bindingProvider = (BindingProvider) port;
        bindingProvider.getRequestContext().put(BindingProvider.ENDPOINT_ADDRESS_PROPERTY, EC2ClientConfig.BROKER_SERVICE_EXTERNAL_EC2_STREAMED);
    ...
    protected ReceiptExternalStreamedBE uploadFile(String fileReference,
                                                String file,
                                                byte[] dataHandler) throws IBrokerServiceExternalEC2StreamedUploadFileStreamedECAltinnFaultFaultFaultMessage {
        ...
        ((BindingProvider) brokerServiceExternalEC2Streamed).getRequestContext().put(Header.HEADER_LIST, headerList);
        ...
        return brokerServiceExternalEC2Streamed.uploadFileStreamedEC(streamedPayloadECBE);
    }

    protected byte[] downloadFile(String reference) throws IBrokerServiceExternalEC2StreamedDownloadFileStreamedECAltinnFaultFaultFaultMessage {
        return brokerServiceExternalEC2Streamed.downloadFileStreamedEC(credentials.getVirksomhetsbruker(), credentials.getVirksomhetsbrukerPassord(), reference, credentials.getEnhet());
    }
