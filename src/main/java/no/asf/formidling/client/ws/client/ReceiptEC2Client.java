 ...
 */
public class ReceiptEC2Client {

    private ObjectFactory objectFactory;

    private Logger log = LoggerFactory.getLogger(this.getClass());

    private IReceiptExternalEC2 receiptExternalEC2;

    private SecurityCredentials credentials;

    private IReceiptExternalEC2 getReceiptClient(Properties signatureProperties) {
        ...
        bindingProvider.getRequestContext().put(BindingProvider.ENDPOINT_ADDRESS_PROPERTY, EC2ClientConfig.RECEIPT_SERVICE_EXTERNAL_EC2);
     ...
     */
    public ReceiptEC2Client(SecurityCredentials credentials) {
        this.credentials = credentials;
        this.objectFactory = new ObjectFactory();
        this.receiptExternalEC2 = getReceiptClient(credentials.getKeyStoreProperties());
    }
     ...
     *                   - SendersReference
     * @return Receipt
     * @throws IReceiptExternalEC2GetReceiptECV2AltinnFaultFaultFaultMessage Exception fra ReceiptExternalEC
     */
    public Receipt getReceipt(Integer receiptId, List<ReceiptReference> references) throws IReceiptExternalEC2GetReceiptECV2AltinnFaultFaultFaultMessage {
        log.info("Getting receipt " + receiptId);

        ReceiptSearch search = getReceiptSearch(receiptId, references);
        return receiptExternalEC2.getReceiptECV2(credentials.getVirksomhetsbruker(), credentials.getVirksomhetsbrukerPassord(), search);
    }
     ...
     * @throws IReceiptExternalEC2GetReceiptListECV2AltinnFaultFaultFaultMessage Exception fra ReceiptExternalEC
     */
    public ReceiptList getReceiptList(Date dateFrom, Date dateTo) throws DatatypeConfigurationException, IReceiptExternalEC2GetReceiptListECV2AltinnFaultFaultFaultMessage {
        log.info("Getting receipt list between " + dateFrom + " to " + dateTo);

        return receiptExternalEC2.getReceiptListECV2(credentials.getVirksomhetsbruker(), credentials.getVirksomhetsbrukerPassord(),
     ...
     * @throws IReceiptExternalEC2UpdateReceiptECAltinnFaultFaultFaultMessage Exception fra ReceiptExternalEC
     */
    public Receipt updateReceipt(Integer receiptId, ReceiptStatusEnum receiptStatus, String arkivReferanse,
                                 List<ReceiptReference> references, String receiptText, List<SubReceipt> subReceipts) throws IReceiptExternalEC2UpdateReceiptECAltinnFaultFaultFaultMessage {
        ...
        return receiptExternalEC2.updateReceiptEC(credentials.getVirksomhetsbruker(), credentials.getVirksomhetsbrukerPassord(), receiptSave);
    }
